...
const kazanOther: TravelLocationCandidateV1 = {
  locationId: 'arvelis:test:station:kazan-other',
  displayName: 'Казань, другая станция',
  type: 'station',
  countryCode: 'RU',
};
...
const unresolvedResolver = new StubResolver(new Map([
  ['Москва', outcome('Москва', [])],
]));
...
assert.deepEqual(
  await new RoutesResolutionService(missingBindingResolver, bindings).resolve(request, context),
  { status: 'blocked', field: 'destination', code: 'provider_binding_missing' },
);

const malicious = { ...request, searchCode: 's2000003' };
const rejectedResolver = new StubResolver(new Map());
assert.deepEqual(
  await new RoutesResolutionService(rejectedResolver, bindings).resolve(malicious, context),
  { status: 'blocked', field: 'origin', code: 'location_resolution_failed' },
);
...
assert.deepEqual(
  await new RoutesResolutionService(abortedResolver, bindings).resolve(request, { ...context, signal: aborted.signal }),
  { status: 'blocked', field: 'origin', code: 'location_resolution_failed' },
);
