...
## Transport Provider Strategy & Adapter Foundation V1 — CURRENT RELEASE GATE
...
- [x] Aviasales Search marked blocked for current early-stage strategy;
- [x] Aviasales Data retained only as future cached insight source;
- [x] implementation push gate PASS;
- [ ] living docs/Security + `docs/47_...` committed;
- [ ] stacked Draft PR #31 final regression PASS;

No real API key, live HTTP traffic, booking or production activation belongs to this foundation slice.

## Next agreed slice — Yandex Rasp Live Adapter V1

Начинается только после зелёного Provider Foundation DoD.

Scope:

1. реальный server-side HTTP client;
2. env-only API key;
3. location resolution;
4. official Yandex request/response mapping;
5. attribution output contract;
6. temporary-cache-only policy;
7. freshness/provenance without invented provider validity;
...
Перед фактической activation ключа повторно проверяются current Yandex official terms, attribution, quota и endpoint access.
