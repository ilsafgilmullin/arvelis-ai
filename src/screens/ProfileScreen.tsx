  ...
  return (
    <div className="content-page">
      ...
      <section className="profile-overview">
        ...
        <div><p className="section-kicker">ПРОФИЛЬ</p><h2>{profileName}</h2><span>Локальный профиль · реальный аккаунт будет подключён позже</span></div>
      </section>
      ...
      <section className="settings-panel">
        ...
        <button className="setting-action setting-action--danger" type="button" onClick={() => setConfirmReset(true)}>
          <ResetIcon /><div><strong>Сбросить локальные данные</strong><span>Удалит локальные диалоги и вернёт стартовые примеры.</span></div><span>Сбросить</span>
        </button>
      </section>

      <section className="security-note">
        <p className="section-kicker">АККАУНТ И БЕЗОПАСНОСТЬ</p>
        <h3>Защищённая учётная запись пока не подключена.</h3>
        <p>Текущая версия не хранит серверную сессию и не просит email, пароль, API-ключи или другие секреты. Полноценные настройки аккаунта появятся вместе с backend-авторизацией.</p>
      </section>
